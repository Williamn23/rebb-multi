...
// Build payment params — link to original product when possible
function buildPayment(acc: any, c: any, overrideAmount?: number) {
  ...
  // If we have the original plan_id, use it directly
  if (c.plan_id) {
    params.plan_id = c.plan_id;
  } else {
    // Create inline plan — include product_id if we have it
...
export async function POST(req: NextRequest) {
  ...
  if (action === "charge_single") {
    ...
    try {
      ...
      // If plan_id fails, retry without it (fallback to inline plan)
      if (c.plan_id) {
        try {
          const fallbackPlan: any = { initial_price: amt, currency: c.currency || "usd", plan_type: "one_time" };
          if (c.product_id) fallbackPlan.product_id = c.product_id;
          const p = await whop.payments.create({
            company_id: acc.whop_company_id,
            member_id: c.member_id,
            payment_method_id: c.payment_method_id,
            plan: fallbackPlan,
          ...
          return NextResponse.json({ success: true, payment_id: p.id, amount: amt, note: "fallback" });
        } catch (err2: any) {
          return NextResponse.json({ error: err2.message || "Charge failed" }, { status: 400 });
        }
      }
